"""Run msft_pmcc_strategy.py backtest on QuantConnect."""
import hashlib, requests, time, os

QC_USER_ID      = "426855"
QC_TOKEN        = "a197cd7a8911f9c32603f0f10601e78d4dbf223de66d161b9551551d28723910"
STRATEGY_FILE   = os.path.join(os.path.dirname(__file__), "msft_pmcc_strategy.py")
PROJECT_ID_FILE = os.path.join(os.path.dirname(__file__), "msft_pmcc_project.txt")
...
def get_or_create_project():
    if os.path.exists(PROJECT_ID_FILE):
        ...
        return pid
    print("Creating new QC project: MSFT PMCC...")
    a, h = auth()
    r = requests.post("https://www.quantconnect.com/api/v2/projects/create",
        auth=a, headers=h, json={"name": "MSFT PMCC Strategy", "language": "Py"})
    data = r.json()
...
def launch(project_id, compile_id):
    ...
    r = requests.post("https://www.quantconnect.com/api/v2/backtests/create",
        ...
        json={"projectId": project_id, "compileId": compile_id,
              "backtestName": "MSFT PMCC — 30DTE + Put Protection"})
...
if __name__ == "__main__":
    print("=== MSFT PMCC — 30DTE + Put Protection ===")
