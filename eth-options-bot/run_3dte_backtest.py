"""
EMA Spread Strategy -- 3 DTE Backtest Using REAL Deribit Data
=============================================================
Same EMA(9/21) hybrid strategy as the weekly backtest, but targeting
options with ~3 days to expiration (DTE 2-4).

Key differences vs weekly (5-10 DTE):
  - Enters every weekday (not just Mondays) -- positions last ~3 days
  - Daily option expiries generated so there's always a 3-DTE chain
  - close_dte=0: let theta decay fully, close on expiry day
  - Higher trade frequency, smaller credits per trade
...
setup_logging("WARNING", "logs/3dte_bt.log")

BASE_URL = "https://www.deribit.com"

# ---------------------------------------------------------------------------
# Deribit data fetchers (same as weekly backtest)
# ---------------------------------------------------------------------------
...
def build_chain(spot, timestamp, base_iv, expiry_days, anchor_spot):
    ...
    T = max(expiry_days, 0.1) / 365.0   # min 0.1 days to avoid zero T

    # Strike grid anchored to spot: tighter grid for short DTE
    raw = [anchor_spot * (1 + i * 0.02) for i in range(-12, 13)]
...
print("=" * 62)
print("  EMA Spread Strategy  --  3 DTE Backtest (Real Deribit Data)")
print("  Period : Mar 2024 - Mar 2026  |  Capital: $2,200")
print("  DTE    : 2-4 days  |  Entry: every weekday")
print("=" * 62)
...
# ---------------------------------------------------------------------------
# Step 3: Build real-data option chains (daily expiries for 3 DTE coverage)
# ---------------------------------------------------------------------------
print(f"\n[3/4] Building 3-DTE option chain snapshots from real data...")
parquet_dir = "data/3dte_backtest/parquet"

# Daily expiry dates -- ensures there's always a chain expiring ~3 days out
expiry_dates = [start_dt + timedelta(days=d) for d in range(1, 731)]

# Anchor spot per expiry = closing price 3 days before expiry
expiry_anchor: dict = {}
for exp in expiry_dates:
    anchor_date = (exp - timedelta(days=3)).date()
...
for _, row in merged.iterrows():
    ...
    for exp in expiry_dates:
        dte = (exp - dt).days
        if dte < 1 or dte > 7:    # only short-dated: 1-7 day chains
            continue
...
print(f"      Done. Stored {built} daily snapshots with short-dated chains.")

# ---------------------------------------------------------------------------
# Step 4: Run 3 DTE EMA spread backtest
# ---------------------------------------------------------------------------
print(f"\n[4/4] Running 3-DTE EMA spread backtest on real Deribit data...")

cfg = EMASpreadConfig(
    fast_ema=9,
    slow_ema=21,
    target_dte_min=2,             # 3 DTE window: 2-4 days
    target_dte_max=4,
    ...
    close_dte=0,                  # hold to expiry day (max theta)
    ...
    entry_every_day=True,         # enter every weekday (3 DTE = new trade every 3 days)
    account_size=2200.0,
...
if trades:
    ...
    print("\n" + "=" * 80)
    print("  Per-Trade Log  (3 DTE | Real ETH Prices + Real Deribit IV)")
    print("=" * 80)
    rows = []
    for t in trades:
        ...
        dte_at_entry = (t.expiry - t.entry_time).days if t.expiry else 0
        rows.append({
    ...
    os.makedirs("data/3dte_backtest", exist_ok=True)
    df.to_csv("data/3dte_backtest/trade_history.csv", index=False)
    print(f"\n  Trade history -> data/3dte_backtest/trade_history.csv")

    # ETH price summary
    print("\n" + "=" * 62)
    print("  Real ETH Price Data (weekly sample)")
    print("=" * 62)
    ...
    print(weekly[["date", "close", "iv_pct"]].rename(
        columns={"close": "ETH_close", "iv_pct": "Deribit_IV%"}
