"""
EMA Spread Strategy -- 7 DTE Backtest Using REAL Deribit Data
=============================================================
Same EMA(9/21) hybrid strategy targeting ~7 days to expiration.
Entry every weekday, close at DTE=1.
"""
...
setup_logging("WARNING", "logs/7dte_bt.log")
...
print("=" * 62)
print("  EMA Spread Strategy  --  7 DTE Backtest (Real Deribit Data)")
print("  Period : Mar 2024 - Mar 2026  |  Capital: $2,200")
print("  DTE    : 5-9 days  |  Entry: every weekday")
print("=" * 62)
...
print(f"\n[3/4] Building 7-DTE option chain snapshots...")
parquet_dir  = "data/7dte_backtest/parquet"
# Every 2 days as expiry to cover 5-9 DTE window on any weekday entry
...
print(f"\n[4/4] Running 7-DTE EMA spread backtest...")
cfg = EMASpreadConfig(
...
if trades:
    ...
    os.makedirs("data/7dte_backtest", exist_ok=True)
    df_out.to_csv("data/7dte_backtest/trade_history.csv", index=False)
    print(f"\n  Trade history -> data/7dte_backtest/trade_history.csv")
