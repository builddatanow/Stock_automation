"""
0 DTE Parameter Sweep Backtest
================================
...
# ---------------------------------------------------------------------------
# Data fetchers (copied from run_0dte_sydney.py to avoid top-level code)
# ---------------------------------------------------------------------------
...
setup_logging("WARNING", "logs/0dte_sweep.log")
...
# ---------------------------------------------------------------------------
def build_merged():
    print("[1/2] Fetching ETH price history...")
    ohlcv = fetch_eth_ohlcv(START - timedelta(days=60), END)
    ohlcv["date"] = ohlcv["timestamp"].dt.date
    print(f"      {len(ohlcv)} candles")

    print("[2/2] Fetching historical IV...")
    iv_df = fetch_historical_iv()
    if not iv_df.empty:
        # fetch_historical_iv() already has a "date" column
...
def summarise(results, window_label, tp, sl, strength):
    ...
    # Estimate USD using average spot from metrics
    avg_spot = results.get("metrics", {}).get("avg_spot", 2500)
    total_usd = total_eth * avg_spot
...
def main():
    ...
    print("=" * 65)
    print("  0 DTE Parameter Sweep — Mar 2024 to Mar 2026")
    ...
    merged    = build_merged()
    cache_dir = "data/0dte_sweep_cache"
    ...
    # ── Top 20 by Net PnL ──────────────────────────────────────────────────
    print("\n" + "=" * 95)
    print("  TOP 20 — Ranked by Net PnL")
    print("=" * 95)
    ...
    # ── Top 10 by Sharpe ───────────────────────────────────────────────────
    print("\n" + "=" * 95)
    print("  TOP 10 — Ranked by Sharpe Ratio")
    print("=" * 95)
    print(hdr)
    print("-" * 95)
    ...
    # ── Save ───────────────────────────────────────────────────────────────
    out_path = "data/0dte_sweep_results.csv"
