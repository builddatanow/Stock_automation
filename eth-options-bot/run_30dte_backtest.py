"""
EMA Spread Strategy -- 30 DTE Backtest Using REAL Deribit Data
==============================================================
Same EMA(9/21) hybrid strategy targeting ~30 days to expiration.
Entry every weekday, close at DTE=7 (tastytrade-style: exit early to capture
theta decay before gamma risk accelerates near expiry).
Delta: short 0.25-0.35 (higher for more premium), wing 0.10-0.15 (wider protection).
"""
...
setup_logging("WARNING", "logs/30dte_bt.log")
...
def build_chain(spot, timestamp, base_iv, expiry_days, anchor_spot):
    ...
    # Wider strike range for 30 DTE — more potential price movement
    raw = [anchor_spot * (1 + i * 0.03) for i in range(-20, 21)]
    strikes = sorted(set(round(k / 50) * 50 for k in raw if k > 0))

    def smile_iv(K):
        m = (K - spot) / spot
        # Flatter smile for longer-dated options
        return base_iv * (1 + 2.0 * m**2 - 0.3 * m) if m < 0 else base_iv * (1 + 1.0 * m**2)
...
print("=" * 62)
print("  EMA Spread Strategy  --  30 DTE Backtest (Real Deribit Data)")
print("  Period : Mar 2024 - Mar 2026  |  Capital: $2,200")
print("  DTE    : 25-35 days  |  Entry: every weekday  |  Close: DTE=7")
print("  Delta  : short 0.25-0.35  |  wing 0.10-0.15")
print("=" * 62)
...
print(f"\n[3/4] Building 30-DTE option chain snapshots...")
parquet_dir  = "data/30dte_backtest/parquet"
# Only need expiries every 7 days since we target 25-35 DTE range
# But we still use daily to cover the close_dte=7 exit window properly
expiry_dates = [start_dt + timedelta(days=d) for d in range(1, 731)]

expiry_anchor = {}
for exp in expiry_dates:
    anchor_date = (exp - timedelta(days=30)).date()
...
print(f"\n[4/4] Running 30-DTE EMA spread backtest...")
cfg = EMASpreadConfig(
    fast_ema=9, slow_ema=21,
    target_dte_min=25,
    target_dte_max=35,
    short_delta_min=0.25, short_delta_max=0.35,  # higher delta = more premium
    wing_delta_min=0.10,  wing_delta_max=0.15,   # wider protection wing
    take_profit_pct=0.50, stop_loss_multiplier=1.5,
    close_dte=7,                                  # exit at DTE=7, before gamma risk
    iv_percentile_min=10.0, min_trend_strength=0.003,
    condor_on_low_iv=True,
    ic_short_delta_min=0.20, ic_short_delta_max=0.30,
    ic_wing_delta_min=0.08,  ic_wing_delta_max=0.13,
...
if trades:
    ...
    os.makedirs("data/30dte_backtest", exist_ok=True)
    df_out.to_csv("data/30dte_backtest/trade_history.csv", index=False)
    print(f"\n  Trade history -> data/30dte_backtest/trade_history.csv")
