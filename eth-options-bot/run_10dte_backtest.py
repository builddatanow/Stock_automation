"""
EMA Spread Strategy -- 10 DTE Backtest Using REAL Deribit Data
==============================================================
Same EMA(9/21) hybrid strategy targeting ~10 days to expiration.
Entry every weekday, close at DTE=2.
Delta: same as 7 DTE (short 0.20-0.30, wing 0.08-0.12).
"""
...
setup_logging("WARNING", "logs/10dte_bt.log")
...
def build_chain(spot, timestamp, base_iv, expiry_days, anchor_spot):
    ...
    raw = [anchor_spot * (1 + i * 0.025) for i in range(-16, 17)]
...
print("=" * 62)
print("  EMA Spread Strategy  --  10 DTE Backtest (Real Deribit Data)")
print("  Period : Mar 2024 - Mar 2026  |  Capital: $2,200")
print("  DTE    : 8-12 days  |  Entry: every weekday  |  Close: DTE=2")
print("=" * 62)
...
print(f"\n[3/4] Building 10-DTE option chain snapshots...")
parquet_dir  = "data/10dte_backtest/parquet"
expiry_dates = [start_dt + timedelta(days=d) for d in range(1, 731)]

expiry_anchor = {}
for exp in expiry_dates:
    anchor_date = (exp - timedelta(days=10)).date()
...
print(f"\n[4/4] Running 10-DTE EMA spread backtest...")
cfg = EMASpreadConfig(
...
if trades:
    ...
    os.makedirs("data/10dte_backtest", exist_ok=True)
    df_out.to_csv("data/10dte_backtest/trade_history.csv", index=False)
    print(f"\n  Trade history -> data/10dte_backtest/trade_history.csv")
